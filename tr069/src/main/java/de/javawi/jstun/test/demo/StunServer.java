...
public class StunServer {
        ...
        try {
          ...
          try {
            ...
            if (receiveMH.getType() == MessageHeaderType.BindingRequest) {
              ...
              Username username = (Username)receiveMH.getMessageAttribute(MessageAttributeType.Username);
              if (username != null){
                Device device = deviceRepository.findFirstBySerialNumber(username.getUsername().trim());
                if (device != null){
                  device.setUdpConnectionRequestAddress(receive.getAddress().getHostAddress() + ":" + receive.getPort());
                  device.setLastUdpRequestTime(new Date());
  ...
  public StunServer(int primaryPort, InetAddress primary, int secondaryPort, InetAddress secondary, DeviceRepository deviceRepository)
